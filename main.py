...
def help_message(message):
    bot.send_message(message.chat.id, f"Вносите данные в формате:  add: full_name "
                                      f"identification_number rating auto fines telefon. Используйте только 1 пробел как "
                                      f"разделитель.")

@bot.message_handler(commands=['delete'])
def del_message(message):
    bot.send_message(message.chat.id, "Удалите запись используя del: и введя через пробел id водителя")

@bot.message_handler(commands=['change'])
def help_message(message):
    bot.send_message(message.chat.id, f"Вносите изменения в формате:  change: userid full_name "
                                      f"identification_number rating auto fines telefon. Используйте только 1 пробел как "
                                      f"разделитель.")
